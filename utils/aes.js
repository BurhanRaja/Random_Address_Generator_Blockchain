...
exports.decryptAES = async (encryptedText, key) => {
    // Convert from Base64
    const bytes = new Uint8Array(decode(encryptedText).split("").map((c) => c.charCodeAt(0)));
...
(async () => {
    const text = "end tonight viable energy mother keep one phrase excite evolve exclude carbon";
